 ...
 */
export function Avatar({ name, imageUrl = null, size = 48 }) {
  if (!imageUrl) {
    return (
    <>    
        <div style={{borderRadius: "50%", width: size, height: size, backgroundColor: "#ccc", display: "flex", alignItems: "center", justifyContent:"center"}}>
            <p style={{fontSize: size/2}}>{name[0].toUpperCase()}</p> 
        </div>
        <p>{name}</p>
    </>)
  }
  ...
  return(
  <>
    <div style={{borderRadius: "50%", width: size, height: size, }}>
        <img src={imageUrl} />
    </div>
    <p>{name}</p>
  </>
...
export function Card({ title, children, variant = "default" }) {
  ...
  const estilos = {
    default: {...base, backgroundColor: "white", border: "1px solid #e0e0e0"},
    outlined: {...base, backgroundColor: "transparent", border: "1px solid #999"},
    elevated: {...base, backgroundColor: "white", boxShadow: "0 2px 8px rgba(0,0,0,0.15"},
  };
...
export function ContactCard({name, email, phone, department}) {
  return (
    <div style={{padding: 16, borderRadius: 8, marginBottom: 12, backgroundColor: "white", border: "1px solid #e0e0e0"}}>
      <p>Nombre: {name}</p>
      <p>Email: {email}</p>
      <p>Telefono: {phone}</p>
...
export function DirectorioContactos({users}) {

  return(
    <>
      <p>Cantidad de contactos: {users.length}</p>
      <div>
        {users.map(user => <ContactCard key={user.id} {...user}/> )}
      </div>
    </>
  )
}
